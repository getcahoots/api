...
module.exports = function instantiate () {
    ...
    return {
        ...
        generatePassword: service.generatePassword.bind(service),
...
AccountService.prototype.register = function register (account, callback) {
    ...
    account.created = account.modified = (Date.now() / 1000 | 0);
...
/**
 * Generates a new random plain text password.
 *
 * @returns {string}
 *
 */
AccountService.prototype.generatePassword = function generatePassword (length) {
    ...
    return this.$createSalt().substring(0, length);
};
