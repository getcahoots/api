...
describe('The AccountService', function suite () {
    ...
    it('should be able to register a new account', function test (done) {
        ...
        function onRegister (err, registeredAccount) {
            ...
            expect(registeredAccount.created).to.be(registeredAccount.modified);
    ...
    it('should be able to find all accounts', function test (done) {
        ...
        function onFind (err, accounts) {
            ...
            expect(accounts[0].salt).to.be(undefined);
    ...
    it('should be able to find the superuser', function test (done) {
        var service = services('account');
        ...
        function onRegister (err) {
            ...
            service.findSuperuser(onFindSuperuser);
        }

        function onFindSuperuser (err, superuser) {
            expect(err).to.be(null);

            expect(superuser).not.to.be(undefined);
            expect(superuser.salt).to.be(undefined);
            expect(superuser.email).to.be(account.email);
    ...
    it('should return `undefined` when no superuser is available', function test (done) {
        var service = services('account');

        function onFindSuperuser (err, superuser) {
            expect(err).to.be(null);

            expect(superuser).to.be(undefined);
    ...
    it('should be able to generate a random password', function test (done) {
        var service = services('account');

        expect(service.generatePassword().length).to.be(10);

        expect(service.generatePassword(20).length).to.be(20);
        expect(service.generatePassword(200).length).to.be(40);
